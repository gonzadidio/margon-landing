      ...
      <div className="ad-app flex font-sans">
        ...
        <aside className={`ad-panel border-r ad-line w-60 shrink-0 flex-col p-3.5 gap-1 fixed inset-y-0 left-0 z-40 transition-transform lg:static lg:translate-x-0 ${menu ? 'flex translate-x-0' : 'flex -translate-x-full lg:translate-x-0'}`}>
          <div className="flex items-center gap-2.5 px-2 py-3">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-400 to-emerald-600 grid place-items-center text-white font-extrabold text-sm">M</div>
            <div className="leading-tight">
              <p className="font-bold ad-ink text-[15px]">Margon</p>
              <p className="text-[10px] ad-faint -mt-0.5">panel interno</p>
            </div>
            ...
            {TABS.map(({ id, label, icon: Icon }) => {
              ...
              return (
                <button key={id} onClick={() => go(id)}
                  className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm font-medium transition ${on ? 'bg-primary-50 text-primary-700' : 'ad-muted hover:bg-[#f1f4f2] hover:ad-ink'}`}>
          ...
          <button onClick={onLogout} className="mt-auto flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm ad-muted hover:bg-[#f1f4f2] hover:text-red-600 transition border-t ad-line pt-3">
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          <header className="ad-panel border-b ad-line h-14 flex items-center gap-3 px-4 lg:px-6 sticky top-0 z-20">
            ...
            <span className="ml-auto text-xs px-2 py-1 rounded-full bg-primary-50 text-primary-700 font-medium">margonsoftware.com</span>
          </header>
